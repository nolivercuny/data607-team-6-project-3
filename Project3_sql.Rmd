...
```{r}
jobListingWithId <- dbReadTable(con,"job_listing")
skillsTableDf <- data.frame("job_listing_id","skill")
for (listing in jobListingWithId) {
  for (skill in skillsDf) {
    if(str_detect(listing$description, regex(paste('[^A-Z0-9]',skill,'[^A-Z0-9]',sep = ''),ignore_case = TRUE))==TRUE){
      skillsTableDf %>% add_row(job_listing_id = listing$id, skill = skill)
    }
  }
}
